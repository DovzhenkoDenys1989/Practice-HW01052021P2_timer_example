class Timer {
  ...
  _timeoutHandler = () => {
    ...
    this._timeContainer.innerText = this._msToTime(
  ...
  _msToTime(duration = 0) {
    const seconds = this._getCorrectTimeString(
      ((duration / 1000) % 60).toFixed(3)
    );
    const minutes = this._getCorrectTimeString(
      Math.trunc((duration / (1000 * 60)) % 60)
    );
    const hours = this._getCorrectTimeString(
      Math.trunc(duration / (1000 * 60 * 60))
    );
